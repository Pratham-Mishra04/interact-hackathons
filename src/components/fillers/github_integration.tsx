...
const GithubIntegration = ({ isParticipant = false }: { isParticipant?: boolean }) => {
  return (
    <div className="w-full h-[78px] flex items-center gap-2 bg-white p-2 pr-4 rounded-l-full rounded-r-xl space-y-2">
      ...
      <div className="grow space-y-1">
        <div className="w-full flex items-center gap-2 text-sm font-medium">
          ...
          Github Integrations disabled for this Hackathon.
        </div>
        {isParticipant && (
          <div className="w-full flex items-center gap-2 text-xs">
            <Link className="text-primary_text" size={24} />
            Include your Github Repos in the Project Submission.
          </div>
        )}
      </div>
    </div>
